...
import { Play, Loader2, Clock, Trash2, Database, AlertTriangle, Download, FileJson, Terminal, History as HistoryIcon, Code2 } from 'lucide-react';
    ...
    return (
        <div className="flex h-full">
            ...
            <div className="flex-1 flex flex-col border-r border-border overflow-hidden">
                {/* Editor Area */}
                <div className="h-1/3 flex flex-col bg-surface border-b border-border">
                   <div className="flex items-center justify-between p-2 pl-4 border-b border-border bg-surface-alt">
                        <span className="text-xs font-bold opacity-50 text-text-muted">SQL EDITOR</span>
                    ...
                    <div className="flex items-center gap-4 text-xs font-mono text-text-muted">
                     ...
                     <div className="flex items-center gap-2">
                        ...
                        <button 
                            onClick={handleRunQuery}
                            disabled={running}
                            className="bg-primary hover:brightness-110 text-white px-4 py-1.5 rounded text-xs font-bold uppercase tracking-wider flex items-center gap-2 transition-all"
                        >
                            {running ? <Loader2 className="animate-spin w-3.5 h-3.5" /> : <Play size={14} fill="currentColor" />}
                            Run
                        </button>
                     </div>
                </div>
