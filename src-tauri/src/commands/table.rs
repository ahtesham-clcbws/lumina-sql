...
#[tauri::command]
pub async fn global_search(db: String, term: String, state: State<'_, AppState>) -> Result<Vec<SearchResult>, String> {
   // This was in lib.rs. Moving here as it iterates tables.
   let pool = {
        let pool_guard = state.pool.lock().unwrap();
        pool_guard.as_ref().cloned().ok_or("Not connected")?
    };
    ...
    for table in tables {
         // Get columns
         let columns: Vec<(String, String)> = conn.query(format!("SHOW COLUMNS FROM `{}`.`{}`", db, table))
            .await.map_err(|e| e.to_string())?;
         
         let mut where_clauses = Vec::new();
         for (col_name, col_type) in columns {
             let t = col_type.to_lowercase();
             if t.contains("char") || t.contains("text") || t.contains("int") {
                 where_clauses.push(format!("`{}` LIKE '%{}%'", col_name, term));
             }
         }
         
         if !where_clauses.is_empty() {
             let sql = format!("SELECT COUNT(*) FROM `{}`.`{}` WHERE {}", db, table, where_clauses.join(" OR "));
             let count: Option<u64> = conn.query_first(sql).await.unwrap_or(Some(0));
